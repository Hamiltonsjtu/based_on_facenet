...
def pnet_serving(image, host, port):
    # host = '127.0.0.1'
    # port = 9001
...
def onet_serving(image, host, port):
    # host = '127.0.0.1'
    # port = 9001
...
def rnet_serving(image, host, port):
    # host = '127.0.0.1'
    # port = 9001
...
def detect_face(img, minsize=20, host = '127.0.0.1', port = 5000, portthreshold =None, factor=0.709):
    ...
    for scale in scales:
        ...
        out = pnet_serving(img_y, host, port)
    ...
    if numbox > 0:
        ...
        out = rnet_serving(tempimg1, host, port)
    ...
    if numbox > 0:
        ...
        tempimg = (tempimg - 127.5) * 0.0078125
        tempimg1 = np.transpose(tempimg, (3, 1, 0, 2))
        out = onet_serving(tempimg1, host, port)
        out0 = np.transpose(out[0])
        out1 = np.transpose(out[1])
