...
def _Pnet(weight_path = 'model12.h5'):
    ...
    ### ----- ADD LAYER  ----  ###
    outs = _PNET_post(name='Pnet_output_post')([classifier, bbox_regress, input_shape])
    model = Model(inputs = [input_img, input_shape], outputs = outs)
...
while (True):
    ...
    output = Pnet.predict([img, shape_img])
