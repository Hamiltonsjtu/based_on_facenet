...
def pnet_serving(image, channel):

    stub = prediction_service_pb2_grpc.PredictionServiceStub(channel)
    request = predict_pb2.PredictRequest()

    request.model_spec.name = 'MTCNN_zs'
    request.model_spec.signature_name = 'pnet_ouput'
    ...
    result_tmp = stub.Predict(request, 10.0)
    result = result_tmp.outputs['pnet_face'].float_val
...
def rnet_serving(image):
    ...
    request.model_spec.signature_name = 'rnet_ouput'
...
def onet_serving(image):
    ...
    request.model_spec.signature_name = 'onet_ouput'
...
def detect_face(img, channel, minsize=20, threshold=None, factor=0.709):
    ...
    for scale in scales:
        ...
        out = pnet_serving(img_y, channel)
...
def main(args):
    ...
    tf.app.flags.DEFINE_string('MTCNN_zs', '192.168.1.254:8501', 'PredictionService host:port')
    channel = grpc.insecure_channel(FLAGS.MTCNN_zs)
    ...
    for file in files:
        ...
        bboxes, points = detect_face(img, channel)
...
def parse_argument(argv):
    ...
    parser.add_argument('--image_dir', type=str, help='Directory with unaligned images.',
                        default=r'F:\peoples_baidu\xijinping_baidu')
