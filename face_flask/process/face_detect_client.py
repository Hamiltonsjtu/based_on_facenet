...
def rnet_serving(image, channel):
    stub = prediction_service_pb2_grpc.PredictionServiceStub(channel)
    request = predict_pb2.PredictRequest()
...
#         tempimg = (tempimg - 127.5) * 0.0078125
        print('====rnet_serving start')
        out = rnet_serving(tempimg1, channel)
    ...
    if numbox > 0:
        ...
        tempimg1 = np.transpose(tempimg, (3, 1, 0, 2))
#         tempimg = (tempimg - 127.5) * 0.0078125
        print('====onet_serving start')
        out = onet_serving(tempimg1, channel)
        out0 = np.transpose(out[0])
        out1 = np.transpose(out[1])
...
def main(image_dir):
    FACE_CHANNEL = grpc.insecure_channel('192.168.1.254:9900')
    ...
    for file in files:
        ...
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        bboxes, points = detect_face(img, FACE_CHANNEL)
        ...
        for index, bbox in enumerate(bboxes):
            ...
            save_file_name = image_dir + '_{}_crop_{}{}'.format(file_name, index, file_ext)
...
if __name__ == '__main__':
    pic_name = 'C:\\Users\\kc\\Desktop\\face\\'
    main(pic_name)
